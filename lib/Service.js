...
var Service = oo({
 ...
 /******************************************************************************
  ...
  _defineExpressRouteForOperation: function(app, method, endpoint, operation) {
    ...
    // Attach operation to request and response
    var attach_operation = (req, res, next) => {
      req.operation = res.operation = operation
      next()
    }
    ...
    var args = [path, this._authenticate.bind(this, endpoint)]
    if (operation.middleware)
      args = args.concat(attach_operation, operation.middleware, wrapper)
    else
      args = args.concat(wrapper)
