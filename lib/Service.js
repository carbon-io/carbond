  ...
  cmdargs: { // values are nomnom definitions
    ...
    'start-server': {
      ...
      cmdargs: {
        ...
        'serverDestroy': {
          full: 'enable-server-destroy',
          flag: true,
          property: true,
          help: 'track connections and destroy on shutdown (avoids waiting ' +
                'for socket timeout on inactive persistent connections)'
        },
        'serverTimeout': {
          full: 'server-timeout',
          metavar: 'MSECS',
    ...
    if (this.corsEnabled) {
      var optionsFn = function(req, cb) {
        var options = {
          ...
          exposeHeaders: [ // XXX: these should be customizable at the operation level
            'Location'
          ],
