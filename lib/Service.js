...
CARBOND_ENV = _o('env:NODE_ENV') || 'production'
...
var Service = oo({
    ...
    'start-server': {
      ...
      cmdargs: {
        ...
        'env': {
          ...
          property: true,
          default: CARBOND_ENV,
          help: 'set the mode in which your server will run (note: this will ' +
                'default to the value of NODE_ENV environment variable if ' +
                'present)'
        },
        'gracefulShutdown': {
          ...
          help: 'wait for all connections to terminate before exiting (note: if ' +
                '"--env=production" this will default to "yes", unless ' +
                'explicitly specified, and "no" otherwise)  [' +
                CARBOND_ENV === 'production' + ']'
        },
        'serverSocketTimeout': {
  ...
  _C: function() {
    ...
    // mode
    this.env = CARBOND_ENV

    // socket timeout/destruction
    this.gracefulShutdown = CARBOND_ENV === 'production'
  ...
  start: function(options, cb) {
    ...
    this.gracefulShutdown = options['start-server'].gracefulShutdown || this.env === 'production'
