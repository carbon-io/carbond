  ...
  'start-server': {
    ...
    cmdargs: {
      'port': {
        ...
        metavar: 'PORT',
        property: true,
        help: 'the port on which to bind. set to zero for a random port'
      },
      'hostname': {
        abbr: 'n',
...
var Service = oo({
  ...
  /*****************************************************************************
   * @constructs Service
   * @description Service is the main entry point to a carbond service
   * @memberof carbond
  ...
  _C: function() {
    ...
    /***************************************************************************
     * @property {boolean} [corsEnabled=true]
  ...
  _init: function() {
    ...
    this._initializeRequestLoggers()
  },
  ...
  _initializeRequestLoggers: function() {
    if (this.requestLoggers)
      for (let requestLogger of this.requestLoggers)
        requestLogger._initialize()
  },
  ...
  start: function(options, cb) {
    ...
    var initAndListen = function() {
      ...
      self._initializeResponseMiddleware()
      ...
      // initialize limiter
      self._initializeServiceLimiter()
  ...
  stop: function(cb) {
    ...
    if (cluster.isMaster || !(self._stopping || self._stopped)) {
      ...
      if (!self.cluster || cluster.isWorker) {
        ...
        try {
          if (!self.gracefulShutdown) {
            if (_.isFunction(self._server.destroy)) {
              self.logInfo('Service destroying outstanding server connections')
              self._server.sync.destroy()
            } else {
              self.logWarning('Service expected destroy to be enabled, but it ' +
                              'wasn\'t. Attempting to close instead...')
 ...
 /******************************************************************************
  ...
  /*****************************************************************************
   * @method _initializeServiceLimiter
   * @description Initializes the service level limiter
   * @returns {undefined}
   */
  _initializeServiceLimiter: function() {
    var limiter = this._initializeLimiter(this.serviceLimiter, this)
    if (!_.isNil(limiter)) {
      var args = [limiter.process.bind(limiter)]
      if (!limiter.preAuth) {
        args.unshift(this._authenticate.bind(this, this))
      }
      this._expressApp.use.apply(this._expressApp, args)
    }
  },
   ...
   * @description Initializes a {@link carbond.limiter.Limiter} instance
  ...
  _initializeLimiter: function(limiter, node) {
    if (!_.isNil(limiter)) {
      ...
      */
      if (!(limiter instanceof _o('./limiter/Limiter'))) {
        throw new Error("Service.limiter should be an instance of Limiter, got: " + this.limiter)
      }
  ...
  _initializeResponseMiddleware: function() {
    const self = this

    self._expressApp.use((_, res, next) => {
        const chunks = []
  ...
  _handleError: function(err, res) {
    if (err instanceof this.errors.HttpError) {
      if (res) {
        ...
        // XXX: should this test >= 500?
        if (_.isNumber(err.code) && err.code === 500) {
          this.logError(err.stack)
          if (this.env === 'production') {
            // XXX: there is no mechanism to distinguish what should be sanitized at the moment.
            //      take the conservative approach here and remove the message in this case
            delete responseOutput.message
          }
        }
  ...
  _defineExpressRouteForOperation: function(app, method, endpoint, operation) {
    ...
    var middleware_metadata = function(req, res, next) {
      req.getRequestMetadata = function() {
        const result = { }

        for (let parameterName in operation.getAllParameters()) {
          const parameter = operation.parameters[parameterName]

          if (!result[parameter.location])
            result[parameter.location] = { }

          result[parameter.location][parameter.name] =
            parameter.extractParameterValueFromRequest(req)
        }

        result.timestamp = req.timestamp
        result.path = req.route.path
        result.user = {
          _id: req.user._id,
          account: {
      ...
      res.getResponseMetadata = function() {
        const result = { }

        const response = operation.responses[this.statusCode]

        result.body = response.extractBodyFromResponse(this)
    ...
    var middleware_requestLogging = function(req, res, next) {
      if (endpoint.service.requestLoggers)
        res.on('finish', () => {
          for (let requestLogger of endpoint.service.requestLoggers)
            requestLogger.log(
              requestLogger.createLogRecord(
                req.getRequestMetadata(),
                res.getResponseMetadata()
              )
            )
        })
    ...
    var args = [path, this._authenticate.bind(this, endpoint), middleware_metadata, middleware_requestLogging, wrapper]
