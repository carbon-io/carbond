...
var logger = logging.getLogger('carbon-io.carbond').child('Service')
...
//      console.log(e.stack) // XXX this is not really ok -- need to know which type exception
  ...
  _configureLogger: function() {
    ...
    if (_.isString(clusterId)) {
      template = '[{{{time}}}]{{{clusterId}}}{{{hostname}}}:{{{name}}}:{{{level}}}: {{{object}}}\n'
    } else {
      template = '[{{{time}}}] {{{hostname}}}:{{{name}}}:{{{level}}}: {{{object}}}\n'
    }
    var config =  {
      ...
      streams: [
        o({
          ...
          template: template,
          formatters: {
            time: logging.streams.Template.formatters.time.iso,
            level: logging.streams.Template.formatters.level.name,
            msg: function(val) {
