...
var Service = oo({
      ...
      if (self.cluster) {
        if (cluster.isMaster) {
          ...
          cluster.on('message', function(msg) {
            self.logInfo('got message: ' + JSON.stringify(msg))
            switch (msg.type) {
              case 'error':
                switch (msg.msg) {
                  case 'initAndListen':
                    ...
                    self.logInfo('number of workers: ' + self._workers.length)
                    delete self._workers[msg.id]
                    if (_.keys(self._workers).length === 0) {
                      self.logInfo('all workers dead, stopping.')
  ...
  stop: function(cb) {
    ...
    if (!(self._stopping || self._stopped)) {
      self._stopping = true
