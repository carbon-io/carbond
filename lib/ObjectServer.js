...
module.exports = oo({
 ...
 /**********************************************************************
  ...
  _initializeStaticRoutes: function() {
    ...
    app.use(adminRoot + "/apidoc", express.static(__dirname + "/../www/apidoc"))
    app.use(adminRoot + "/swagger-ui", express.static(__dirname + "/../node_modules/swagger-ui/dist"))

    // Swagger descriptor endpoint
    var self = this
    app.get(adminRoot + "/api-docs", function(req, res) {
      var descriptor = {}
      try {
        ...
        if (operation.responses) {
          ...
          res.json = function(body) {
            ...
            if (res.statusCode < 400) { // If not error, validate response
              ...
              if (response) {
                var validationResult = self.jsonSchemaValidator.validate(output, response.schema)
