...
module.exports = oo({
 ...
 /**********************************************************************
   ...
   */        
  start: function() { 
    this.logInfo("ObjectServer starting...")
    ...
    this.logInfo("ObjectServer listening on port " + this.port)
  ...
  _makeHttpsOptions: function() {
    ...
    if (options) {
      ...
      if (options.serverKeyPassword) {
        httpsOptions.passphrase = options.serverKeyPassword
        delete options.serverKeyPassword // XXX how does this help exactly?
      }
    }
