...
module.exports = oo({
  ...
  _initializeAuthenticator: function() {
    ...
    if (self.authenticator) {
      ...
      self._expressApp.use(function(req, res, next) {
        var user = self.authenticator.authenticate(req)
        if (user) {
          req.user = user
        } // XXX reject properly
        next()
  ...
  _isOperationAuthorized: function(user, endpoint, method) {
    ...
    // if this is an OPTIONS request accept
    if (method === "options") { // XXX we should review this but CORS pre-flight needs it
      return true
    }
