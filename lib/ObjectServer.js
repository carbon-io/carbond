...
module.exports = oo({
 ...
 /**********************************************************************
  ...
  _initializeDatabaseConnections: function() { 
    // initialize dbs
    if (this.dbUri) {
      this.logInfo("ObjectServer initializing connection to db: " + this.dbUri)
      try {
        this.db = connect(this.dbUri)
      } catch(e) {
        throw new Error("Could not connect to " + this.dbUri + " - Reason: " + e.message)
      }
    }
    ...
    if (this.dbUris) {
      ...
      for (var dbname in this.dbUris) {
        this.logInfo("ObjectServer initializing connection to db: " + this.dbUris[dbname])
