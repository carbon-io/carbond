...
module.exports = oo({
 ...
 /**********************************************************************
  ...
  _initializeExpress: function() {
    ...
    this._expressApp.use(bodyParser.json()) // XXX do we only want json() and urlencoded()
    this._expressApp.use(bodyParser.urlencoded({ extended: true })) // XXX do we only want json() and urlencoded()
