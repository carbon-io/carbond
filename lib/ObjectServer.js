...
module.exports = oo({
  ...
  _main: function(options) { 
    ...
    if (options['gen-static-docs']) {
      var exitVal = this._genStaticDocumentation(options['static-docs-flavor'], 
                                                 options['static-docs-out'],
                                                 options['static-docs-single-page'])
    ...
    var OutStream = {
      write: function(rec) {
        ...
        if (rec.level === bunyan.INFO || rec.level === bunyan.WARN) {
          if (isString) {
            console.error('[%s]  %s: %s', rec.time, levels[rec.level], rec.object)
          } else {
            console.error('[%s]  %s: %j', rec.time, levels[rec.level], rec.object)
          }
        } else {
          if (isString) {
            console.error('[%s] %s: %s', rec.time, levels[rec.level], rec.object)
