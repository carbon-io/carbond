...
module.exports = oo({
  ...
  runSelfTest: function(cb) {
    ...
    self.logInfo("Running self test for this Service")
    if (this.selfTest) {
      this.selfTest.sync.run() 
      self.logInfo("Self test complete")
    } else {
      self.logWarning("No self test defined")
    }
  },
