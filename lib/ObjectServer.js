...
module.exports = oo({
  ...
  _main: function(options) { 
    ...
    if (options['gen-static-docs']) {
      this._genStaticDocumentation(options['static-docs-flavor'])
      process.exit(0)
    }
 ...
 /**********************************************************************
  ...
  cmdargs: { // values are nomnom definitions
    ...
    'gen-static-docs': {
      flag: true,
      help: "generate docs for the api and print to stdout (will exit afterward). " +
            "use the '--static-docs-flavor' switch to choose your flavor (default " +
  ...
  _processAndValidateParameters: function(req, operation, endpoint) {
    ...
    for (var parameterName in parameterDefinitions) {
      ...
      this.logDebug("ObjectServer._processAndValidateParameters(): Value of final parameter '" + parameterName +
                    "' is: " + JSON.stringify(req.parameters[parameterName]))
    }
  },
...
//      console.log(e.stack) // XXX this is not really ok -- need to know which type exception
  ...
  _isOperationAuthorized: function(user, endpoint, method, req) {
    ...
    if (!acl) { // if no acl on endpoint we authorize
      this.logDebug("ObjectServer._isOperationAuthorized(): true - No ACL endpoint defined")
      return true
    }

    var result = false
    try {
      result = acl.hasPermission(user, method, { req: req })
    } catch (e) {
   ...
   */
  _genStaticDocumentation: function(flavor) {
    // change the log level to avoid logging output in the doc stream (remove if
    // log stream set to stderr)
    this._log.level("fatal")
    // this is needed to build routes on Collection endpoints
    this._initializeApp()
    generator = _o('./StaticDocumentationGenerator').createGenerator(flavor, this)
    console.log(generator.generateDocs())
  }
})
