...
module.exports = oo({
  ...
  _main: function(options) { 
    ...
    if (options['gen-static-docs']) {
      this._genStaticDocumentation(options['static-docs-flavor'], options['static-docs-out'])
      process.exit(0)
    }
 ...
 /**********************************************************************
  ...
  cmdargs: { // values are nomnom definitions
    ...
    'static-docs-out': {
      default: "README.md",
      metavar: "PATH",
      help: "path to write static docs to (use \"stdout for stdout)"
    }
  },
...
//      console.log(e.stack) // XXX this is not really ok -- need to know which type exception
   ...
   */
  _genStaticDocumentation: function(flavor, docsPath) {
    var docs = _o('./StaticDocumentationGenerator').createGenerator(flavor, this).generateDocs()
    if (!docsPath || docsPath === 'stdout') {
      console.log(docs)
    }
    else {
      fs.writeFileSync(path.resolve(docsPath), docs)
    }
  }
})
