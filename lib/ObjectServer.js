 ...
 /**********************************************************************
  ...
  _authenticate: function(req, res, next) {
    ...
    if (user) {
      ...
      var e = this.errors.Forbidden("Could not authenticate user")
      res.status(e.code).send(e)
    }
  },
...
//XXX      this._expressApp.use(this._authenticate.bind(this))
    }
  },
  ...
  _sanitize: function(value, user, filterSingleValue, filterArrays) {
    try {
      return _o('./security/ObjectAcl').sanitize(value, user, filterArrays, filterArrays)
  ...
  _isOperationAuthorized: function(user, endpoint, method) {
    ...
    // Important to keep this check even though authenticator
    // should reject when no user found. It may incorrectly not 
    // do so
    if (!user) {
      return false
    }
