...
module.exports = oo({
 ...
 /**********************************************************************
  ...
  start: function(cb) { 
    ...
    // If http then we should change user now
    if (self._server instanceof http.Server) {
    ...
    self._server.listen(self.port, function() {
      ...
      // Switch the process user
      if (self._server instanceof https.Server) {
        self._ensureProcessUser()
      }
      ...
      if (cb) {
        cb() // XXX should we take a call back to start() or should we try to sync?
      }
    }) 
  },
  ...
  _ensureProcessUser: function() {
    ...
    if (this.processUser) {
      try {
        ...
        this.logInfo("ObjectServer setting process user to: " + this.processUser)
      } catch (e) {
        this.logFatal("ObjectServer failed to set process user to: " + this.processUser + ", exiting")
        process.exit(1)
      }
      if (process.getuid() == 0) {
  ...
  _initializeHttpServer: function() {
    ...
    this.logDebug("SslOptions: " + this.sslOptions)
    // http or https?
  ...
  _defineExpressRouteForOperation: function(app, method, endpoint, operation) {
    ...
    var f
    // XXX should we do all this in Endpoint _init?
    if (typeof(operation) === 'function') {
      var opFn = operation
      operation = o({
        _type: Operation,
        service: function(req, res) {
          return opFn.call(endpoint, req, res)
        }
      })
  ...
  _makeOperationWrapper: function(operation, endpoint, method) {
    ...
    var wrapper = function(req, res) {
      try {
        ...
        try {
          self._processAndValidateParameters(req, operation)
        } catch (e) {
        ...
        if (self._isOperationAuthorized(req.user, endpoint, method)) {
          ...
          try {
            result = operation.service(req, res) // the actual operation
          } catch (e) {
            self._handleError(e, res)
            return // be defensive
          }
   ...
   */
  _processAndValidateParameters: function(req, operation) {
    ...
    if (schema) {
      ...
      try {
        if (parameterDefinition.location === "body") { 
          ...
          value = EJSON.parse(rawValue) // XXX forgive strings without quotes?
        }
      } catch (pe) {
