...
module.exports = oo({
  ...
  cmdargs: { // values are nomnom definitions
    ...
    'test': {
      command: true,
      help: 'run the test suite (Service.testSuite)',
    }
  },
  ...
  runTestSuite: function(cb) {
    ...
    self.logInfo("Running test suite for this Service")
    if (this.testSuite) {
      this.testSuite.sync.run() 
      self.logInfo("Test suite complete")
    } else {
      self.logWarning("No test suite defined")
    }
  },
