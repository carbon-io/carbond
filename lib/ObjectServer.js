...
module.exports = oo({
  ...
  _startServer: function(options) {
    this.logDebug("ObjectServer._startMain")
  ...
  _genStaticDocs: function(options) {
    ...
    var exitVal = this._genStaticDocumentation(
      options['gen-static-docs']['flavor'],
      options['gen-static-docs']['out'],
      options['gen-static-docs']['single-page'])
 ...
 /**********************************************************************
    ...
    'gen-static-docs': {
      ...
      cmdargs: {
        'flavor': {
          ...
          choices: ['github-flavored-markdown'],
          metavar: 'FLAVOR',
        ...
        'single-page': {
          flag: true,
          help: 'write docs to a single page instead of multiple'
        }
      }
    }
  },
  ...
  start: function(options, cb) {
    ...
    if (this.cluster && cluster.isMaster) {
      logStartup()
      this._workers = {}
      var numWorkers = this.numClusterWorkers === 0 ? os.cpus().length : this.numClusterWorkers
      for (var i = 0; i < numWorkers; i++) {
        var worker = cluster.fork()
        this._workers[worker.id] = worker
      }
      this.logInfo(`ObjectServer started`)
      var isListening = false
      cluster.on('listening', (worker, addrInfo) => {
        if (!isListening) {
          this.logInfo(`ObjectServer listening on ${addrInfo.port}`)
          isListening = true
        }
      })
      cluster.on('exit', (worker, code, signal) => {
        this.logInfo(`ObjectServer worker ${worker.id} exited`)
      })
    }
    else {
      if (self.cluster) {
        // reinitialize the logger
        delete self._bunyanLog
        self._loggerConf = {name: 'ObjectServer'}
      }
      else {
        logStartup()
      }
      ...
      if (this.doStart.length <= 1) {
        this.doStart(options)
        // listen
        self._listen(cb)
      } else if (this.doStart.length == 2) {
        // Only pass cb if doStart wants it
        this.doStart(options, function() {
          // listen
          self._listen(cb)
        })
      } else {
        throw new Error("Unexpected arity to doStart(). Expecting 0, 1, or 2 formal parameters. Got: " +
                        this.doStart.length)
      }
    }
  },
...
//      console.log(e.stack) // XXX this is not really ok -- need to know which type exception
  ...
  /**********************************************************************
   * _genStaticDocumentation
   */
  _genStaticDocumentation: function(flavor, docsPath, singlePage) {
    return _o('./StaticDocumentationGenerator').createGenerator(
              flavor, this).generateDocs(docsPath, singlePage)
  }
})
