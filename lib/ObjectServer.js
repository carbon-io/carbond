 ...
 /**********************************************************************
  ...
  start: function(cb) { 
    ...
    self._server.listen(self.port, function() {
      ...
      // Register signal handler for exit XXX is this is optimal place to do this? Not sure.
    ...
    if (schema) {
      ...
      try {
        if (parameterDefinition.location === "body") { 
          ...
          // Be forgiving of unquoted strings. If schema defines parameter as a string and the value 
          // is a string without quotes, auto-wrap. Otherwise just EJSON parse
          if ((schema.type === 'string') && (typeof(rawValue) === 'string')) {
      ...
      // Validate the schema the JSON Schema validator
