...
module.exports = oo({
  ...
  _main: function(options) { 
    console.log(this.constructor.prototype.cmdargs)
  ...
  start: function(cb) { 
    ...
    self._expressApp.listen(self.port, function() {
      ...
      if (self.processUser) {
        ...
        if (process.getuid() == 0) {
          self.logFatal("ObjectServer should not be running as root, exiting. Consider setting " + 
...
//      result.requestCert = options.requireClientCerts
 //     result.secureProtocol = 'TLSv1_method' // XXX still needed?
