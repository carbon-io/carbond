...
module.exports = oo({
  ...
  _startServer: function(options) {
    this.logDebug("ObjectServer._startServer")

    // Register signal handlers 
    // XXX or do we only want to register this after doStart() has been called 
    // since presumably doStop() will undo anything done in doStart()?
  ...
  _genStaticDocs: function(options) {
    this.logDebug("ObjectServer._genStaticDocs")
 ...
 /**********************************************************************
  ...
  start: function(options, cb) {
    ...
    // XXX: do we want to allow auto-restarting of child processes?
    if (self.cluster && cluster.isMaster) {
      ...
      cluster.on('listening', function (worker, addrInfo) {
        if (!isListening) {
          ...
          self.logInfo('ObjectServer listening on ' + addrInfo.port)
          isListening = true
        }
      })
      cluster.on('exit', function (worker, code, signal) {
        self.logInfo('ObjectServer worker ' + worker.id + 'exited')
      })
    } else {
      ...
      if (self.doStart.length === 2) { // no cb but cb expected. XXX do we want to error here?
        throw new Error("doStart() expects a callback but none supplied to start()")
  ...
  _listen: function(cb) {
    ...
    self._server.listen(self.port, self.hostname, function() {
      ...
      if (self.cluster) {
        self.logInfo('ObjectServer worker ' + cluster.worker.id + ' started')
      } else {
  ...
  stop: function(cb) {
    ...
    self.logInfo("ObjectServer stopping...")
    // XXX what should we do here? Close db connections and unlisten?
    if (self.doStop.length === 1) { // no cb but cb expected XXX do we want to throw error here?
      throw new Error("doStop() expects a callback but none supplied to stop()")
    } if (self.doStop.length < 1) { // no cb and none expected
      self.doStop()
      self.logInfo("ObjectServer stopping HTTP server")
      self._server.sync.close()
      self.logInfo("ObjectServer closing database connections")
      self._closeDatabaseConnections()
    }

    self.logInfo("ObjectServer stopped")
  },
 ...
 /**********************************************************************
  ...
  _defineExpressRouteForOperation: function(app, method, endpoint, operation, parentEndpointsPath) {
    if (!operation.service) { // XXX leave this for now
      console.error("ERROR ***** this should not happen contact will")
      console.error(method)
      console.error(typeof(operation))
      return
    }
  ...
  _makeOperationWrapper: function(operation, endpoint, method) {
    ...
    var wrapper = function(req, res) {
      ...
      try {
        ...
        // Wrap response.json to validate in case where result is writtedn to response
        if (operation.responseSchema) {
          var f = res.json.bind(res)
          res.json = function(body) {
            var validationResult = self.jsonSchemaValidator.validate(body, operation.responseSchema)
            if (!validationResult.valid) {
              throw new self.errors.InternalServerError("Output did not validate against schema: " + 
                                                        EJSON.stringify(body) + " " + 
                                                        EJSON.stringify(operation.responseSchema))
            }
            return f(EJSON.serialize(body))
          }
        }
...
//        console.log("++")
  //      console.log(result)
        if (result !== undefined) {
          ...
          // Else assume response will be written to. Would be nice to be able to let ops resturn 
          // undefined to cause us to end() here. Not sure best way. 
          // XXX maybe null response with $undefined in the schema. Or just always turn null to undefined?
        }
      } catch (e) {
        self._handleError(e, res)
        return
  ...
  _processAndValidateParameter: function(req, parameterDefinition) {
    ...
    if (schema) {
      ...
      try {
        if (parameterDefinition.location === "body") {
          ...
          // comes back as {}
          value = EJSON.deserialize(rawValue) // XXX deserialize can silently fail here (ex bad $date)
        } else {
      ...
      var validationResult = schemaValidator.validate(EJSON.serialize(value), schema)
      if (!validationResult.valid) {
    ...
    // If there was a resolver defined on the OperationParameter then use it
    if (parameterDefinition.resolver) {
      req.parameters[name] = parameterDefinition.resolver(req.parameters[name])
    }
  },
