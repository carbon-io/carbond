...
module.exports = oo({
  ...
  _initializeChildEndpoints: function() {
    ...
    _.forIn(childEndpoints, function(childEndpoint, path) {
      if (path[0] === ':') {
        if (!self.endpoints[self.idPathParameter]) {
          self.endpoints[self.idPathParameter] = self._makeObjectSubEndpoint()
        }
        var i = path.indexOf('/')
        if (i !== -1) {
          var newPath = path.substring(i + 1)
          self.endpoints[self.idPathParameter].endpoints[newPath] = childEndpoint
