...
module.exports = oo({
  ...
  _C: function() {
    ...
    this.userOwned = false // XXX add to docs
  ...
  _serviceInsert: function(req) {
    ...
    // assign default acl to object
    if (this.userOwned) {
      this._assignDefaultAcl(obj, req.user)
    }
  ...
  _initializeSave: function() {
    if (this.supportsSave) {
      ...
      var self = this
      this.endpoints[':id'].put = { // we intentionally do not define this with Endpoint def above
        params: {
  ...
  _initializeGetObject: function() {
    if (this.supportsGetObject) {
      ...
      var self = this
      this.endpoints[':id'].get = { // we intentionally do not define this with Endpoint def above
        service: function(req) {
  ...
  _initializeRemoveObject: function() {
    if (this.supportsRemoveObject) {
      ...
      var self = this
      this.endpoints[':id'].delete = { // we intentionally do not define this with Endpoint def above
        params: {
   ...
   * _assignDefaultAcl
   */
  _assignDefaultAcl: function(obj, user) {
    if (obj.__acl__) {
      // if this is a userOwned collection then the acl owner must be authenticated user
      // XXX why cant we do this and not be owner?
      if (obj.__acl__.owner !== user._id) {
