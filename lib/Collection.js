...
module.exports = oo({
   ...
   */        
  _init: function() {
    // Patch up idPathParameter if needed to add ':' in front
    if (this.idPathParameter) {
      if (this.idPathParameter[0] !== ':') {
        this.idPathParameter = ':' + this.idPathParameter
      }
    }
  ...
  _initializeFindObject: function() {
    if (this.supportsFindObject) {
      // create a sub-endpoint for this.idPathParameter with a 'get' method

      if (!this.endpoints[this.idPathParameter]) {
        this.endpoints[this.idPathParameter] = this._makeObjectSubEndpoint()
      }
      ...
      var self = this
      this.endpoints[this.idPathParameter].get = o({
        _type: './Operation',
        endpoint: self.endpoints[this.idPathParameter],
        parameters: {
          id: {
        ...
        service: function(req) {
          return self.findObject(req.parameters.id, { req: req })
        }
      })
    }
  },
  ...
  _initializeSaveObject: function() { 
    if (this.supportsSaveObject) {
      // create a sub-endpoint for this.idPathParameter with a 'put' method
      if (!this.endpoints[this.idPathParameter]) {
        this.endpoints[this.idPathParameter] = this._makeObjectSubEndpoint()
      }
      ...
      var self = this
      this.endpoints[this.idPathParameter].put = o({
        _type: './Operation',
        endpoint: self.endpoints[this.idPathParameter],
        parameters: {
          id: {
            description: "Object _id to update",
            location: "path",
            schema: idSchema,
            required: true
          },
  ...
  _initializeUpdateObject: function() {
    if (this.supportsUpdateObject) {
      // create a sub-endpoint for this.idPathParameter with a 'patch' method
      if (!this.endpoints[this.idPathParameter]) {
        this.endpoints[this.idPathParameter] = this._makeObjectSubEndpoint()
      }
      ...
      var self = this
      this.endpoints[this.idPathParameter].patch = o({
        _type: './Operation',
        endpoint: self.endpoints[this.idPathParameter],
        parameters: {
          id: {
  ...
  _initializeRemoveObject: function() {
    if (this.supportsRemoveObject) {
      // create a sub-endpoint for this.idPathParameter with a 'delete' method
      ...
      var self = this
      this.endpoints[this.idPathParameter].delete = o({
        _type: './Operation', 
        endpoint: self.endpoints[this.idPathParameter],
        parameters: {
          id: {
    ...
    return o({
      ...
      acl: o({ // new acl that delegates to the CollectionAcl on this Collection
        ...
        hasPermission: function(user, permission, env) {
          ...
          if (permission === 'get') {
            return self.acl.hasPermission('findObject', env)
          }
          if (permission === 'put') {
            return self.acl.hasPermission('saveObject', env) 
          }
          if (permission === 'patch') {
            return self.acl.hasPermission('updateObject', env) 
          }
          if (permission === 'delete') {
            return self.acl.hasPermission('removeObject', env)
          }
          if (permission === 'head') {
            return self.acl.hasPermission('findObject', env)
          }
