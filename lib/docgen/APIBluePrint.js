...
var APIBPGenerator = oo({
  ...
  _ctorName: 'APIBPGenerator',

  /*****************************************************************************
   * @constructs APIBPGenerator
   * @description Generates api-blueprint style static documentation
   * @memberof carbond.docgen
   * @extends carbond.docgen.StaticDocumentationGenerator
   */
  _C: function() {
  ...
  _fixPathParameterAndAppendQueryParameters: function(endpoint) {
    var path_ = endpoint.path
    var parts = path_.split(path.sep)
    parts = _.map(parts, function(p) {return _.startsWith(p, ":") ? ("{"+p.substring(1)+"}") : p})
    var jointPaths = parts.join(path.sep)
    var queryParams = _.get(endpoint.operations[0], 'parameters.query')
    if (queryParams) {
      var queryParamsString = '{?' + queryParams.map(param => param.name).join() + '}'
      jointPaths += queryParamsString
    }
   ...
   * @returns {string} -- The api-blueprint text for the service
   */
  ...
  /*****************************************************************************
   * @method generateDocs
   * @description Generates api-blueprint docs and writes them to standard out or
   *              a file
   * @note api-blueprint docs will always be generated as a single page
   * @param {string} docsPath -- The path to write docs to
   * @param {list} options -- An array of options as specified by the user
  ...
  _registerHandlebarsHelpers: function() {
    ...
    handlebars.registerHelper('APIBPURLTemplate', function(endpoint, options) {
      return new handlebars.SafeString(self._fixPathParameterAndAppendQueryParameters(endpoint))
    })
