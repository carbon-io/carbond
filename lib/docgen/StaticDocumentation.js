...
var StaticDocumentationGenerator = oo({
  ...
  _generateParameterDescriptors: function(parameters, inheritedParameters, path_) {
    ...
    var existingPathParams = _.map(_parameters.path, function(val) {
      return val.name
    })
  ...
  _generateResponseDescriptors: function(responses) {
    ...
    return _.sortBy(_.map(responses, descriptor), function(r) {
      return r.name
    })
  },
  ...
  _renderBody: function(schema) {
    ...
    var self = this
    if (schema.enum) {
      return '( ' + _.map(schema.enum, function(e) {
        return '"' + e + '"'
      }).join(' | ') + ' )'
    } else if (schema.oneOf) {
      var schemas = _.map(schema.oneOf, function(t) {
        return self._renderBody(t)
      })
      if (_.find(schemas, function(s) {
        return s.indexOf('\n') >= 0
      })) {
      ...
      if (_.isArray(schema.type)) {
        return '( ' + _.map(schema.type, function(t) {
        ...
        switch (schema.type) {
          case 'object':
            ...
            if (numKeys === 0) {
              ...
              return '{\n' + self._lpad(_.map(schema.properties, function(v, k) {
                return k + ': ' + self._renderBody(v)
              }).join(',\n')) + '\n}'
            }
          case 'array':
            if (!schema.items) {
