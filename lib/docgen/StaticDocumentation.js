...
var StaticDocumentationGenerator = oo({
  ...
  _generateOperationDescriptor: function(method, operation, path_) {
    ...
    return {
      ...
      requests: self._generateRequestDescriptors(method, operation.parameters, inheritedParameters, path_),
      responses: self._generateResponseDescriptors(operation.responses)
    }
  },

  _generateRequestDescriptors: function(method, parameters, inheritedParameters, path) {
    const parameterDescriptors = this._generateParameterDescriptors(operation.parameters, inheritedParameters, path)
      ...
      return [
        {
          ...
          requestSchemaElements: {
            children: this._recurseSchemaDescriptors(request_schema)
          }
        }
      ]
    }
