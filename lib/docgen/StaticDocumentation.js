...
var StaticDocumentationGenerator = oo({
  ...
  _generateOperationDescriptor: function(method, operation, path_) {
    ...
    const parameterDescriptors = self._generateParameterDescriptors(operation.parameters, inheritedParameters, path_)
    ...
    return {
      ...
      parameters: {
        path: parameterDescriptors.path,
        query: parameterDescriptors.query
      },
      requests: [
        {
          parameters: {
            header: parameterDescriptors.header,
            body: parameterDescriptors.body
          },
          requestSchemaElements: self._generateRequestSchemaDescriptors(method, operation.parameters)
        }
      ],
  ...
  _generateURIParametersDescriptor: function(parameters, inheritedParameters) {
  ...
  _generateResponseDescriptors: function(responses) {
    ...
    var descriptor = function(response) {
      ...
      if (Math.floor(response.statusCode / 100) === 2)
        desc.responseSchemaElements = self._generateResponseSchemaDescriptors(responses)
