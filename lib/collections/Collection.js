...
var Collection = oo({
    ...
    if (this.supportsFindObject) {
      ...
      objectSubEndpoint.get = o({
        ...
        service: function(req, res) {
          ...
          params.id = params.context[self.idParameter]
          params.context = _.omit(params.context, self.idParameter)
          params = _.assignIn(
  ...
  preSaveObjectOperation: function(config, req, res) {
    ...
    return {
      context: _.omit(req.parameters, this.idParameter),
      options: config.options
    }
  },
  ...
  postUpdateOperation: function(result, config, req, res) {
    result = this._getResult(result)
    if ((!(config.opConfig.returnsUpsertedObjects && result.created) &&
        (!_.isInteger(result.val) || result.val < 0))) {
    ...
    if (config.opConfig.supportsUpsert && result.created) {
      var headers = this._get201Headers(result.val, req)
      res.status(201)
      res.header('Location', headers.location)
      res.header(this.idHeader, headers[this.idHeader])
      if (config.opConfig.returnsUpsertedObjects) {
        return result.val
      }
    }
    return {n: result.val}
    ...
    if (this.supportsUpdate) {
      ...
      this.patch = o({
        ...
        service: function(req, res) {
          ...
          params.update = params.context.body
          params.context = _.omit(params.context, 'body')
          params = _.assignIn(
  ...
  postUpdateObjectOperation: function(result, config, req, res) {
    result = this._getResult(result)
    if ((!(config.opConfig.returnsUpsertedObject && result.created) &&
        (!_.isInteger(result.val) || result.val < 0))) {
      throw new Error('Update returned ' + result.val.toString() + ' instead of an integer')
    }
    // the upsert/created verification case should be handled by the response schema
    ...
    if (config.opConfig.supportsUpsert && result.created) {
      ...
      res.header('Location', headers.location)
      res.header(this.idHeader, headers[this.idHeader])
      if (config.opConfig.returnsUpsertedObject) {
        return result.val
      }
    }
    return {n: result.val}
    ...
    if (this.supportsUpdateObject) {
      ...
      objectSubEndpoint.patch = o({
        ...
        service: function(req, res) {
          ...
          params.id = params.context[self.idParameter]
          params.update = params.context.body
          params.context = _.omit(params.context, 'body', self.idParameter)
          params = _.assignIn(
  ...
  _get201Headers: function(result, req) {
    ...
    var _path = req.path
    if (req.parameters[this.idParameter] === path.basename(_path)) {
