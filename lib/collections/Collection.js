        ...
        service: function(req, res) {
          ...
          if (body) {
            ...
            if (result === true) { // This was an upsert, success
              ...
              // No need to EJSON serialize here. String and ObjectId turn out how we want. 
              // Other EJSON (like Date) not allowed.
              var location = req.path // _id is already a part of the path since this is a object operation
              res.header("Location", location)
              return null
            } else if (result === null || result === undefined) { // Update case, success
              return null // Must convert undefined to null for Service to know to commit the response.
            } else if (result === false) { // Update case, not found
              throw new this.endpoint.service.errors.NotFound("_id: " + id)
            }

            return result // If this is a type that does not conform to schema error will occur there
          }
        }
      })
    }
