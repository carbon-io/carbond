...
module.exports = oo({
  ...
  _generateEndpointOperationForInsert: function() {
    if (this.supportsInsert) {
      ...
      // Schema. We deal with this slightly differently than other operations. We delete _id in some cases.
      ...
      var defaultResponses = [
        {
          statusCode: 201,
          description: "Returns the object inserted, along with the URL of the newly inserted object " +
            "in the Location header of the response.",
          schema: self.schema || self._DEFAULT_SCHEMA, 
          headers: ['Location']
        }
      ]
      ...
      self.post = {
        description: config.description || "Insert an object into this Collection.",
        parameters: {
          "body" : { // XXX can we stop need body -- is swagger bug fixed yet?
            ...
            schema: (self.idRequiredOnInsert ? self._normalizeSchema(schema) : self._unrequireIdPropertyFromSchema(schema))
          }
        },          
        responses: self._makeResponses('insert', config, defaultResponses, true),
        
        service: function(req, res) {
          var obj = req.parameters.body // XXX make obj instead of body (for name)? Must be body for Swagger 2.0 UI bug
          if (!obj) {
            throw new this.endpoint.service.errors.BadRequest("Body required (JSON)")
          }
          ...
          res.status(201) 
          // No need to EJSON serialize here. String and ObjectId turn out how we want. 
  ...
  _generateEndpointOperationForFind: function() {
    if (this.supportsFind) {
      ...
      var defaultResponses = [
        {
          statusCode: 200,
          description: "Returns an array of objects. Each object has an _id and possible additional properties.",
          schema: {
            type: 'array',
            items: self.schema || self._DEFAULT_SCHEMA
          }
        }
      ]
  ...
  _generateEndpointOperationForSaveObject: function() { 
    if (this.supportsSaveObject) {
      ...
      objectSubEndpoint.put = o({
        ...
        _type: '../Operation', 
        description: config.description || "Save an object to this Collection. Will insert if does not exist.",
        parameters: _.assignIn({ // Merge defined config parameters with body parameter.
          body: { // XXX make obj instead of body (for name)? Must be body for Swagger 2.0 UI bug
            ...
            schema: self._validateSchemaHasId(self._normalizeSchema(config.saveSchema || 
                                                                    self.schema || 
                                                                    self._DEFAULT_SCHEMA)),
            required: true
          }
        }, config.parameters),
        responses: self._makeResponses('saveObject', config, defaultResponses, false),
        ...
        service: function(req, res) {
          ...
          if (body) {
            if (EJSON.stringify(id) !== EJSON.stringify(body._id)) { // XXX Is there more efficient compare?
              throw new self.endpoint.service.errors.BadRequest("Path id must be equal to body._id")
            }
            var result = self.saveObject(body, { req: req, res: res }) 
            if (res.statusCode === 201) { // was an upsert
              // No need to EJSON serialize here. String and ObjectId turn out how we want. 
              // Other EJSON (like Date) not allowed.
  ...
  _makeResponses: function(op, config, defaultResponses, validateResponseSchemasHaveId) {
    ...
    var responses = (config && config.responses) || defaultResponses || []
    ...
    if (_.includes(['findObject', 'updateObject', 'removeObject'], op)) {
      responses.push({
        ...
        schema: self._DEFAULT_ERROR_SCHEMA
      })
    }
  ...
  _applyView: function(obj, viewName) {
    ...
    if (!view) {
      throw new this.endpoint.service.errors.BadRequest("Unknown view: " + viewName)   
    }
