...
module.exports = oo({
  ...
  _generateEndpointOperations: function() {
    ...
    this.idHeader = this.idHeader || this.defaultIdHeader
  ...
  _generateEndpointOperationForInsert: function() {
    if (this.supportsInsert) {
      ...
      var example = config.example || (config.insertSchema ? undefined : self.example)
      ...
      var defaultResponses = [
        {
          ...
          schema: config.returnsInsertedObject ? (self.schema || self.defaultSchema) : {type: 'Undefined'},
      ...
      self.post = {
        ...
        noDocument: config.noDocument || false,
        parameters: {
        ...
        service: function(req, res) {
          ...
          var location = path.normalize(path.join(req.path, result._id.toString()))
          res.header('Location', _.trimEnd(location, '/'))
          res.header(self.idHeader, EJSON.stringify(result._id))
          // Must convert undefined to null for Service to know to commit 
          // the response.
          return config.returnsInsertedObject ? result : null 
        }
      }
    }
  },
  ...
  _generateEndpointOperationForFind: function() {
    if (this.supportsFind) {
      ...
      if(self.example) {
        defaultResponses[0].example = self.example
      }
  ...
  _generateEndpointOperationForSaveObject: function() { 
    if (this.supportsSaveObject) {
      ...
      var bodyParam = { // XXX make obj instead of body (for name)? Must be body for Swagger 2.0 UI bug
        location: "body",
      ...
      var example = config.example || (config.saveSchema ? undefined : self.example)
      if(example) {
        ...
        service: function(req, res) {
          ...
          if (body) {
            ...
            if (result === true) { // This was an upsert, success
              ...
              var location = path.normalize(req.path) // _id is already a part of the path 
                                                      // since this is a object operation
              res.header('Location', _.trimEnd(location, '/'))
              res.header(self.idHeader, EJSON.stringify(id))
              return null
            } 

            // Update case, success. Save will throw if cannot save
            res.status(204)
            return null // Must return null for Service to know to commit the response.
          } else {
  ...
  _makeObjectSubEndpoint: function() {
    ...
    if (schema && schema.properties && schema.properties._id) {
