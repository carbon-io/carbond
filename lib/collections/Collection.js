...
var Collection = oo({
  ...
  _C: function() {
     ...
     *                                     appropriate (e.g. {@link carbond.collections.Colleciont.insert})
     */
  ...
  configureInsertOperation: function() {
    ...
    var insertSchema = config.insertSchema || config.parameters.objects.schema || this.schema
  ...
  configureInsertObjectOperation: function() {
    ...
    var insertObjectSchema = config.insertObjectSchema ||
  ...
  configureSaveObjectOperation: function() {
    ...
    var saveObjectSchema = config.saveObjectSchema ||
                           config.parameters.object.schema ||
                           this.schema
  ...
  configureUpdateOperation: function() {
    ...
    // schema
    var updateSchema = config.updateSchema ||
  ...
  configureUpdateObjectOperation: function() {
    ...
    var updateObjectSchema = config.updateObjectSchema ||
                             config.parameters.update.schema ||
                             {type: 'object'}
  ...
  _makeResponses: function(op, config, defaultResponses, validateResponseSchemasHaveId) {
    ...
    if (_.isObjectLike(config) &&
      ...
      // FIXME: we should merge `config.responses` at the end instead of replacing responses.
      //        this would allow the implementer to override fields (like description) on
      //        the default responses and allow for manipulation of error responses.
