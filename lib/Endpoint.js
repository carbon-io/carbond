...
var EndpointInitError = require('./util/errors').EndpointInitError
 ...
 * }
 */
var Endpoint = oo(mixins.multimixin({
  ...
  _C: function() {
     ...
     * @description The URI path that routes to this endpoint. This is built
     *              during service initialization and will overwrite any value
     ...
     *              the response {@link carbond.OperationResponse.schema}
     *              corresponding to the current response code. Note, the top-level
     *              {@link carbond.Service} and the {@link carbond.Operation} being
     *              invoked also have this property, and, if any are false, output
  ...
  _init: function() {
    try {
      ...
      if (!_.isNil(this.getService())) {
        this.getService().logError(e)
      }
      this.setSanityCheckError(new EndpointInitError(e, this, this.path))
    }
  },
  ...
  _initializeOperation: function(method) {
    ...
    // sanity check the operation (this will get thrown when endpoint sanity is
    // checked during endpoint initialization in Service
    operation.isSane()
  },
