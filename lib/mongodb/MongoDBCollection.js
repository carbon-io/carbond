...
module.exports = oo({
  ...
  saveObject: function(obj, reqCtx) {
    try {
      // XXX: we do not use leafnode.Collection.saveObject here so we can determine whether of not
      //      this results in an upsert
      ...
      if (result.result.upserted && result.result.upserted.length > 0) { // Save resulted in upsert
        ...
        var success = (result.result.n !== 0)
        if (!success) {
          return false // We return false to indicate the object was not found
        }
        return null // We use null to indicate success
      }
    } catch (e) {
      throw new this.service.errors.InternalServerError(e.message)
    }
