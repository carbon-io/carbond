...
var Limiter = oo({
  ...
  sendUnavailable: function(res, message, retryAfter) {
    if (_.isUndefined(res)) {
      throw new TypeError('expected a {@link Response} object buy got undefined')
    }
...
var TooBusyLimiter = oo({
  ...
  _init: function() {
    ...
    if (this.useFiberPoolSize) {
      self.absMaxOutstandingReqs = Math.round(
        fibers.getFiberPoolSize() + this.fiberPoolAllowedOverflow * fibers.getFiberPoolSize())
    }
  ...
  fn: function(req, res, next) {
    ...
    if (numReqs > self.maxOutstandingReqs || toobusy_) {
      ...
      return self.sendUnavailable(res, 'The server is too busy, please try back later.')
    }
