...
module.exports = oo({
  ...
  hasPermission: function(user, permission) {
    ...
    if (dynamicEntries.length > 0) { // XXX we may just want to make them group entries so they need to pass all of them
  ...
  and: function(acl) {
    ...
    return o({
      ...
      hasPermission: function(user, permission) {
        return self.hasPermission(user, permission) && acl.hasPermission(user, permission)
      }
    })
  },
