...
module.exports = oo({
  ...
  /*****************************************************************************
   * @method initialize
   * @description Initializes the authenticator. Called by {@link prop:carbond.Service.start}
   ...
   * @method isRootUser
   * @abstract
   * @description Checks if a user is root.
   * @param {object} user -- An object representing a user
   * @returns {boolean} -- `true` if the user is determined to be root, `false` otherwise.
   */
   ...
   * @abstract
   * @description Gets the names of the request headers where authentication
   *              details can be found. Should be implemented by subclasses, for example:
   *              {@link prop:carbond.security.ApiKeyAuthenticator}
   * @returns {string[]} -- An array of strings representing request headers.
   */
