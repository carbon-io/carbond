...
module.exports = oo({
  ...
  _init: function() {
    ...
    // checks to see if user._id is the same as the object owner as tracked
    // by the owner property of this acl
    var self = this
    ...
    self.groupDefinitions[self.OWNER_SPEC] = function(user) {
      ...
      return EJSON.stringify(self.owner) === EJSON.stringify(user._id) 
    }
  },
   ...
   * sanitize
   */       
  sanitize: function(user, filterSingleValue, filterArrays) {
    return sanitize(this.object, user, filterSingleValue, filterArrays)
  }
})
 ...
 * to read
 */
function sanitize(value, user, filterSingleValue, filterArrays) {
  var result = doSanitize(value, user, filterArrays)
 ...
 * doSanitize
 */
function doSanitize(value, user, filterArrays) {
  if (!value) {
    return value
  }
  ...
  if (value.constructor === Array) {
    result = doSanitizeArray(value, user, filterArrays)
  } else if (typeof(value) === 'object') {
    result = doSanitizeObject(value, user, filterArrays)
  }
 ...
 * doSanitizeArray
 */
function doSanitizeArray(arr, user, filterArrays) {
  var result = []
  var processedElem
  
  arr.forEach(function(elem) {
    if (typeof(elem) === 'object') {
      processedElem = doSanitize(elem, user, filterArrays) // recurse
      ...
      result.push(doSanitize(elem, user, filterArrays)) // otherwise recurse for prims and array
    }
  })
 ...
 * doSanitizeObject
 */
function doSanitizeObject(obj, user) {
  var result = obj
  
  var acl
  var aclDatum = obj.__acl__
  if (aclDatum) {
    acl = o(aclDatum, null, module.exports)
    acl.object = obj
