...
module.exports = oo({
  ...
  generateSwaggerDescriptor: function(objectserver) {
    var descriptor = {
      ...
      "basePath": objectserver.basePath,
      "schemes": ["http"],
      "consumes": ["application/json"],
      "produces": ["application/json"],
      "paths" : this._generateEndpointPaths(objectserver.endpoints, objectserver.basePath)
    }
   ...
   */
  _generateEndpointPaths: function(endpoints, basePath, result) {
    result = result || {}

    basePath = basePath || ""
    if (endpoints) {
      for (var path in endpoints) {
        this._generateEndpointPath(endpoints[path], basePath + "/" + path, result)
      }
    }
