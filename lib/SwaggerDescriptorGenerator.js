...
module.exports = oo({
  ...
  generateSwaggerDescriptor: function(objectserver) {
    var descriptor = {
      ...
      "basePath": objectserver.apiRootPath,
      "schemes": ["http"],
      "consumes": ["application/json"],
      "produces": ["application/json"],
      "paths" : this._generateEndpointPaths(objectserver.endpoints, objectserver.apiRootPath)
    }
   ...
   */
  _generateEndpointPaths: function(endpoints, apiRootPath, result) {
    result = result || {}

    apiRootPath = apiRootPath || ""
    if (endpoints) {
      for (var path in endpoints) {
        this._generateEndpointPath(endpoints[path], apiRootPath + "/" + path, result)
      }
    }
  ...
  _generateOpParameters: function(endpoint, method) {
    ...
    if (pathParameters) {
      pathParameters.forEach(function(param) {
        result.push({
          name: param.substring(1), // XXX hack
          description: param.substring(1),
          required: false,
            in: "path"
        })
      })
